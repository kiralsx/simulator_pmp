...
from utils import JobInfo, NodeInfo, job_name_map
...
class Cluster(object):
    ...
    def __init__(self, workload, policy, num_nodes, ngpus_per_node, allow_pmp=False):
            ...
            for job in self.jobs:
                if job.submission_time <= self.current_time and job.completion_time is None:
                    ...
                    if isinstance(self.policy, WeightedMIPPolicy) or isinstance(self.policy, GavelPolicy) or isinstance(self.policy, WeightedMIPFIXPolicy):
                        job_allocation = allocations[job.name]
            ...
            "submitted_jobs": [
                {
                    ...
                    "placement": {job.current_cluster: job.placement} if job.app_name not in job_name_map or not self.allow_pmp else job.placement,
