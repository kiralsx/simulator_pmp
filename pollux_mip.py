...
import os
from speedup import *
from utils import cluster_name_map, job_name_map, order_cluster
...
class WeightedMIPPolicy(object):
    def __init__(self, p_fairness = 0.5, restart_penalty=30, 
                    ...
                    timeshare_penalty_window=None, sched_interval=60, pmp=False, hete=False):
    ...
    def populate_valid_configs(self, cluster_num_nodes, cluster_num_gpus):
        ...
        # order cluster name
        self.cluster_ordering = order_cluster(self.cluster_ordering)
    ...
    def _compute_goodputs_pmp(self, job_info):
        goodputs = []
    ...
    def optimize_mip(self, jobs, nodes, prev_allocations):
        ...
        # print(f"speedup matrix:\n{final_speedup_matrix}")
        ...
        if self.apply_timeshare_penalty:
            ...
            job_allocs, cluster_allocs = self.__solve_mip_rescaled(final_speedup_matrix, num_gpus, job_weights, 
                                                                   jobnames, realloc_factors, migrate_factors, job_info=jobs)
        ...
        # merge allocs
        job_placements = {}
        for job_name, alloc in job_allocs.items():
            if alloc is None:
                job_placements[job_name] = None
            else:
    ...
    def __solve_mip_rescaled(self, speedup_matrix, num_gpus, job_weights = None, jobnames=None, 
                             realloc_factors=None, migrate_factors=None, timeshare_penalties=None, job_info=None):
        # ones vec
        ...
        ones_configvec = np.ones((num_configs if not self.allow_pmp else num_configs + len(self.pmp_configs), 1), dtype=np.float32)
        ...
        for i, jobname in enumerate(jobnames):
            prev_alloc = self.prev_allocs.get(jobname, None) # one hot vector
            prev_cluster = self.prev_cluster.get(jobname, None) # list of cluster_name
            if self.allow_pmp and jobname in job_name_map: # pmp job
                continue
            else:
                same_cluster_selector = cluster_selectors[prev_cluster[0]] if prev_cluster else cluster_selectors["none"]
                migrate_cluster_selector = ~same_cluster_selector
                
                # rescale speedups projecting reallocations / migrations
                if prev_cluster is not None:
                    # NOTE: only take non-pmp configs ids
                    if self.allow_pmp:
                        prev_alloc = prev_alloc[:prev_alloc.shape[0]-len(self.pmp_configs)]
        ...
        if not self.allow_pmp:
            ...
            for i, jobname in enumerate(jobnames):
                ...
                if appname in job_name_map:
                    goodputs = self._compute_goodputs_pmp(job_info[jobname])
                    if np.max(goodputs) == 0:
                        raise Exception(f'{jobname} cannot run')
                    speedup_matrix_[i, :speedup_matrix_.shape[1]-len(self.pmp_configs)] = 0
                    speedup_matrix_[i, speedup_matrix_.shape[1]-len(self.pmp_configs):] = goodputs
                    speedup_matrix_[i] /= np.min(speedup_matrix_[i, np.nonzero(speedup_matrix_[i])])

                    # rescale speedups projecting reallocations for pmp jobs
        ...
        # slow sub-cluster
        for cluster_id, cluster in enumerate(self.cluster_ordering):
            ...
            if not self.allow_pmp:
                ...
                # NOTE: add gpu usage in pmp configs
                assert x.shape[1]-len(self.pmp_configs) > end_offset - 1
                num_gpu_used = cp.sum(x[:, start_offset : end_offset] @ config_ngpus) \
        ...
        for i, jobname in enumerate(jobnames):
            ...
            if np.sum(soln) == 0:
                ...
                if self.allow_pmp:
                    # NOTE: make sure non-pmp jobs will never pick pmp configs, and pmp jobs will only pick pmp configs
                ...
                if self.allow_pmp and job_info[jobname].app_name in job_name_map:
                    config_ = self.pmp_configs[alloc_config_idx - (x.shape[1] - len(self.pmp_configs))]
                    for cluster_id, cluster_name in enumerate(self.cluster_ordering):
                        ngpus = config_[cluster_id]
                        if ngpus == 0:
                            continue
                        nnodes = 1 if ngpus <= self.cluster_ngpus_per_node[cluster_name] else ngpus // self.cluster_ngpus_per_node[cluster_name]
            ...
            self.prev_allocs[jobname] = soln
            self.prev_cluster[jobname] = None if job_allocs[jobname] == None else list(job_allocs[jobname].keys())
            assert jobnames[i] == jobname

        # print(f'### cluster allocs:')
        # for cluster_name in cluster_allocs:
        #     print(cluster_name, cluster_allocs[cluster_name])
